...
def test_部分結果復元の検証_中断前に確定済みの施設が今回分と合流して出力され復元件数も合算される(tmp_path):
    """7.2の出力側: 前回の中断実行までに``SapaPartialStore``へ確定済みの施設
    (再開実行の``SiteCollectResult``には現れない)が、``restored_features``経由で
    今回分と同じ都道府県バケットへ合流し、GeoJSON出力に含まれることを検証する。
    復元された件数マップ("unknown"バケット除外を含む)も今回分と合算される。
    """
    output_dir = tmp_path / "geo-json"

    restored = _feature(name="復元施設", source_url="https://site-a.example/restored", pref=_TOKYO)
    site_a_result = SiteCollectResult(
        site_key="site-a",
        features=(_feature(name="今回施設", source_url="https://site-a.example/new", pref=_TOKYO),),
        listed_urls=frozenset({"https://site-a.example/restored", "https://site-a.example/new"}),
        skipped_counts={"13": 1},
        geocoded_counts={},
    )
    ...
    results = run_prefectures(
        ...
        restored_skipped_counts={"13": 2, "unknown": 5},
        restored_geocoded_counts={"13": 1},
        output_dir=output_dir,
    )

    assert isinstance(results[0], SapaPrefectureResult)
    assert results[0].scraped_count == 2
    assert results[0].skipped_count == 3  # 今回1 + 復元2("unknown"は都道府県へ帰属させない)
    assert results[0].geocoded_count == 1

    written = read_geojson(output_dir / build_geojson_filename(_TOKYO, FacilityKind.SAPA))
    names = {f.properties.name for f in written}
    assert names == {"復元施設", "今回施設"}
    restored_written = next(f for f in written if f.properties.name == "復元施設")
    # 復元施設も今回の実行で確認済みの施設としてACTIVE・confirmed_atが更新される。
    assert restored_written.properties.status is FacilityStatus.ACTIVE
    assert restored_written.properties.last_confirmed_at == _CONFIRMED_AT


def test_部分結果復元の検証_今回改めて成功した施設は復元分ではなく今回の新しい結果が優先される(tmp_path):
    """``_select_restorable_features``の重複排除: 同一``source_url``の施設が
    復元分と今回分の両方に存在する場合、今回の新しい結果のみが出力される
    (05の``michinoeki.runner``の「合算時はsource_urlで重複を排除し新結果を優先」
    と同じ規律)。
    """
    output_dir = tmp_path / "geo-json"
    url = "https://site-a.example/1"

    restored_stale = _feature(name="古い名称", source_url=url, pref=_TOKYO)
    site_a_result = SiteCollectResult(
        site_key="site-a",
        features=(_feature(name="新しい名称", source_url=url, pref=_TOKYO),),
        listed_urls=frozenset({url}),
        skipped_counts={},
        geocoded_counts={},
    )
    ...
    results = run_prefectures(
        ...
        _ALL_SITES,
        _CONFIRMED_AT,
        restored_features=[restored_stale],
        output_dir=output_dir,
    )

    assert isinstance(results[0], SapaPrefectureResult)
    assert results[0].scraped_count == 1

    written = read_geojson(output_dir / build_geojson_filename(_TOKYO, FacilityKind.SAPA))
    assert [f.properties.name for f in written] == ["新しい名称"]


def test_部分結果復元の検証_失敗サイト帰属の復元施設は出力へ合流せず前回施設の現状維持と二重出力しない(tmp_path):
    """失敗サイトの隔離との整合: 一覧取得に失敗したサイトに帰属する復元施設を
    出力へ合流させると、``_split_previous_by_site_attribution``が現状維持で
    合流させる前回施設と同一``source_url``が二重出力されうる。失敗サイト帰属の
    復元施設は除外され、前回施設が現状維持のまま1件だけ出力されることを検証
    する(除外分は部分結果キャッシュに残り、当該サイトが成功した実行で改めて
    合流する)。
    """
    output_dir = tmp_path / "geo-json"
    url_b = "https://site-b.example/1"
    ...
    filename = build_geojson_filename(_TOKYO, FacilityKind.SAPA)
    write_geojson([previous_b], filename, output_dir=output_dir)

    # 前回の中断実行でサイトBから収集済みだった施設B(今回サイトBは一覧失敗)。
    restored_b = _feature(name="施設B(復元)", source_url=url_b, pref=_TOKYO)
    ...
    results = run_prefectures(
        ...
        _ALL_SITES,
        _CONFIRMED_AT,
        restored_features=[restored_b],
        output_dir=output_dir,
    )

    assert isinstance(results[0], SapaPrefectureResult)
    assert results[0].scraped_count == 0

    written = read_geojson(output_dir / filename)
    assert len(written) == 1
    # 現状維持: 前回施設がそのまま出力され、復元分による上書き・二重出力は無い。
    assert written[0].properties.name == "施設B"
    assert written[0].properties.last_confirmed_at == _PREVIOUS_CONFIRMED_AT


def test_部分結果復元の検証_成功サイト帰属でも今回のlisted_urlsに無い復元施設は再確認扱いされず前回状態のまま削除判定に委ねる(
    tmp_path,
):
    """レビューで検出した回帰: 中断前に収集確定した施設が、再開までの間に実際
    にサイトの一覧から消えていた場合、``_select_restorable_features``が
    ``failed_site_keys``のみで復元施設を選別すると、一覧取得に成功した
    サイトに帰属する復元施設は無条件で「今回確認済み」として合流してしまい、
    ``merge_with_previous``の9.2削除判定(listed_urlsに無ければ削除状態へ
    遷移)を迂回してACTIVE・last_confirmed_at更新のまま復活してしまう。

    今回はサイトAの一覧取得は成功したが、復元施設(サイトA帰属)のURLは今回の
    listed_urlsに含まれない(=一覧から消えた)状況を作り、復元施設が合流せず、
    前回出力がそのまま通常の削除判定(listed_urlsに無い→DELETED)を受ける
    ことを検証する。
    """
    output_dir = tmp_path / "geo-json"
    url_a = "https://site-a.example/vanished"
    previous_a = _feature(
        name="施設A", source_url=url_a, status=FacilityStatus.ACTIVE, last_confirmed_at=_PREVIOUS_CONFIRMED_AT
    )
    filename = build_geojson_filename(_TOKYO, FacilityKind.SAPA)
    ...
    # 前回の中断実行でサイトAから収集済みだった施設A。今回はサイトAの一覧
    # 取得自体は成功するが、施設Aのdetail_urlはもう一覧に含まれない
    # (listed_urlsに無い = サイトから消えた)。
    restored_a = _feature(name="施設A(復元)", source_url=url_a, pref=_TOKYO)
    site_a_result = SiteCollectResult(
        site_key="site-a",
        features=(),
        listed_urls=frozenset({"https://site-a.example/other"}),
        skipped_counts={},
        geocoded_counts={},
    )
    ...
    written = read_geojson(output_dir / filename)
    assert len(written) == 1
    # 復元施設による無条件の再確認(ACTIVE・confirmed_at更新)は起きず、
    # listed_urlsに無いため通常の削除判定でDELETEDへ遷移する。
    assert written[0].properties.status is FacilityStatus.DELETED
    assert written[0].properties.last_confirmed_at == _PREVIOUS_CONFIRMED_AT
...
def test_範囲外復元施設の検証_より広い範囲の中断分が部分結果に残る場合_狭い範囲で全成功してもクリアされない(
    tmp_path, monkeypatch
):
    """レビューで検出した回帰: より広い範囲(例: 関東地方)の実行が中断し、
    範囲内の複数都道府県分の施設が``SapaPartialStore``へ確定済みの状態で、
    より狭い範囲(例: 東京都のみ)で再開して全成功した場合、``run_prefectures``
    は``scope_prefectures``(この場合は東京都のみ)しか処理しないため、範囲外
    (神奈川県)の復元施設はどこにも出力されないまま``run_scope``の7.3クリア
    条件(``failed_site_keys``・``failed_prefecture_codes``がいずれも空)を
    満たしてしまい、収集済みだったデータが永久に失われる。

    神奈川県帰属の復元施設をあらかじめ``SapaPartialStore``へ直接追記して
    「より広い範囲の中断」を模擬し、東京都のみのスコープで全成功する
    ``run_scope``を実行しても、resume/partial_storeがクリアされず、神奈川県の
    復元施設が部分結果に残り続けることを検証する。
    """
    output_dir = tmp_path / "geo-json"
    tokyo_detail_url = "https://site-a.example/tokyo-1"
    tokyo_address = f"{_TOKYO.name_ja}千代田1-1"
    ...
    resume_store = ResumeStore(state_dir=tmp_path / ".resume")
    resume_key = "sapa-test-out-of-scope-restore"
    resume = UrlResumeTracker(resume_key, store=resume_store)
    partial_store = SapaPartialStore(store=resume_store)

    # より広い範囲(関東地方)の実行が中断し、神奈川県(範囲外)の施設だけが
    # 部分結果に確定済みの状態を模擬する。
    kanagawa_url = "https://site-a.example/kanagawa-1"
    partial_store.add_feature(_feature(name="施設神奈川", source_url=kanagawa_url, pref=_KANAGAWA))
    ...
    written = read_geojson(output_dir / filename)
    assert [f.properties.name for f in written] == ["施設東京"]
    # 神奈川県は範囲外のため出力ファイル自体が作られない。
    assert not (output_dir / build_geojson_filename(_KANAGAWA, FacilityKind.SAPA)).exists()

    # 範囲外の復元施設(神奈川県)が残っているため、全成功であってもresume/
    # partial_storeはクリアされず、次回より広い範囲で再開できるよう維持される
    # (今回新たに収集した東京都分もそのままストアに残る。ストア自体は範囲を
    # 意識しない単一の全体キャッシュのため、クリアを見送るとは「今の内容を
    # 丸ごと維持する」ことを意味する)。
    assert resume_store.load(resume_key) is not None
    reloaded_partial_store = SapaPartialStore(store=resume_store)
    assert {f.properties.source_url for f in reloaded_partial_store.features} == {kanagawa_url, tokyo_detail_url}
