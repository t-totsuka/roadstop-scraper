...
1・2・3・4は``EastSite``単独(東京都、``arealist=3``)で、5は``EastSite``
(成功)と``CentralSite``(一覧取得失敗)の2サイトが同時に関与する長野県
(``prefecture_code="20"``、``EastSite``の``arealist=4``・``CentralSite``の
...
# 東日本(driveplaza.com)のarealist=3(関東)の一覧URL。EastSite._LISTING_URL_TEMPLATE
# と同じ構成(実装を直接importせず、実サイトのURL構成を模したフィクスチャとして
# 独立に構成する)。
...
def test_削除状態遷移の結合検証_実サイトアダプタ経由で削除復帰完全除去と抽出失敗時の現状維持が一貫して動作する(
    ...
    実際の``EastSite``(東京都・``arealist=3``)が一覧HTML→詳細HTMLを実際に
...
def test_サイト失敗隔離の結合検証_実サイトアダプタ経由で中日本の一覧取得失敗時に東日本の収集は継続し中日本の前回施設は現状維持される(
    ...
    ``EastSite``(長野県・``arealist=4``、成功)と実際の``CentralSite``
