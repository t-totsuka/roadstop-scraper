...
from roadstop_scraper.geojson import Coordinate, FacilityKind, build_geojson_filename, find_prefecture, read_geojson
...
def test_run_scopeの検証_処理途中の強制中断から再開した場合_中断前に収集済みの施設もGeoJSON出力に含まれる(
    tmp_path,
):
    """7.2の最終出力側: 上の``collect_site``単位のテストは「中断前の結果が部分
    結果キャッシュに残っていること」までを検証しているが、その部分結果が
    ``run_scope``で読み戻され**GeoJSON出力まで**到達することは検証していなかった
    (読み戻しが無いと、中断前に収集済み=処理済み記録済みの施設Aは再開実行の
    ``SiteCollectResult``に現れず出力から欠落し、全成功時の7.3クリアで収集結果
    ごと破棄される)。1回目の``run_scope``を施設Cの詳細取得中に強制中断させ、
    2回目の``run_scope``(Aの再取得は禁止)が成功した後の出力GeoJSONにA・Cの
    両方が含まれることを検証する。
    """
    url_a = "https://www.driveplaza.com/sapa/1010/4010001/1/"
    url_c = "https://www.driveplaza.com/sapa/1010/4010002/1/"
    listing_html = _listing_html([("テスト東京SA-A(上り)", url_a), ("テスト東京SA-C(上り)", url_c)])
    detail_html_a = _detail_html(name="テスト東京SA-A", road_name="中央自動車道", address="東京都新宿区西新宿2-8-1")
    detail_html_c = _detail_html(name="テスト東京SA-C", road_name="中央自動車道", address="東京都新宿区西新宿2-8-3")

    state_dir = tmp_path / ".resume"
    resume_key = "sapa-test-resume-output"
    store = ResumeStore(state_dir=state_dir)
    output_dir = tmp_path / "geo-json"

    # 1回目: 施設Aの処理完了直後、施設Cの詳細取得でプロセス強制終了を模す。
    session_1 = _StubSession(
        {_EAST_TOKYO_LISTING_URL: listing_html, url_a: detail_html_a},
        raise_by_url={url_c: _SimulatedProcessCrash("疑似クラッシュ")},
    )
    with pytest.raises(_SimulatedProcessCrash):
        run_scope(
            ...
            fetcher=PageFetcher(_fast_config(), session=session_1),
            ...
            partial_result_store=SapaPartialStore(store=store),
        )

    # 中断時点ではGeoJSONは未出力(都道府県処理まで到達していない)。
    filename = build_geojson_filename(_TOKYO, FacilityKind.SAPA)
    assert not (output_dir / filename).exists()

    # 再開(プロセス再起動を模す): 同じ状態ディレクトリから再構築した
    # resume/partial_store。Aのdetail_urlが再取得されたら即座に失敗する。
    session_2 = _StubSession(
        {_EAST_TOKYO_LISTING_URL: listing_html, url_c: detail_html_c},
        forbidden_urls=frozenset({url_a}),
    )
    result = run_scope(
        ScopeSpec(prefecture_code="13"),
        fetcher=PageFetcher(_fast_config(), session=session_2),
        geocoder=_StubGeocoder(Coordinate(longitude=139.60, latitude=35.60)),
        resume=UrlResumeTracker(resume_key, store=store),
        confirmed_at=_CONFIRMED_AT,
        output_dir=output_dir,
        partial_result_store=SapaPartialStore(store=store),
    ...
    # 中断前に収集済みのAが(再取得なしで)部分結果から復元され、今回処理した
    # Cとあわせて両方が最終出力に含まれる(取りこぼしなし)。
    written = read_geojson(output_dir / filename)
    assert {f.properties.source_url for f in written} == {url_a, url_c}
    assert {f.properties.name for f in written} == {"テスト東京SA-A", "テスト東京SA-C"}

    # 全成功のためレジューム状態・部分結果は実ファイルごと消去されている。
