...
def test_保存の検証_置き換えが失敗した場合_既存のindex_jsonが破損しない(
    tmp_path: Path, monkeypatch: pytest.MonkeyPatch
):
    index_path = tmp_path / "index.json"
    original = IndexData(files=(IndexEntry(path="01.geojson", updated_at=_dt(1)),))
    index_store.save_index(original, index_path)
    ...
    with pytest.raises(OSError):
        index_store.save_index(
            IndexData(files=(IndexEntry(path="01.geojson", updated_at=_dt(2)),)),
            index_path,
        )

    assert index_store.load_index(index_path) == original
    assert list(tmp_path.glob("*.tmp")) == []
