...
def test_読み戻しの検証_必須項目のみのFeature列が_往復変換された場合_元のFeature列と一致する():
    # 任意項目省略時(8.1, 8.2, 8.4): to→fromの往復で元のFacilityFeature列と一致することを確認する
    features = [
        _build_feature(
            FacilityProperties(
                name="道の駅テスト",
    ...
    restored = from_feature_collection_dict(to_feature_collection_dict(features))

    assert restored == features


def test_読み戻しの検証_全項目つきFeature列が_往復変換された場合_元のFeature列と一致する():
    # parking各内訳・websites複数件・facilities複数件・direction・DELETED状態と
    # last_confirmed_atを含む全項目が、書き込み時と矛盾しない形で復元されることを確認する(8.1, 8.2, 8.4)
    last_confirmed_at = datetime(2026, 7, 1, 9, 0, tzinfo=UTC)
    ...
    features = [_build_feature(properties)]
...
def test_読み戻しの検証_FeatureCollection辞書が_往復変換された場合_元の辞書と一致する():
    # from→toの往復でも元のFeatureCollection辞書と一致することを確認する(8.4)
    original = to_feature_collection_dict(
        [
            _build_feature(_build_full_properties()),
            _build_feature(
                FacilityProperties(
                    name="道の駅テスト2",
                    kind=FacilityKind.MICHINOEKI,
    ...
    restored = to_feature_collection_dict(from_feature_collection_dict(original))
