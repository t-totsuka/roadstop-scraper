...
def test_変換の検証_必須項目のみの施設が_出力された場合_任意項目のキーを省略する():
    ...
    props = to_feature_collection_dict([_build_feature(properties)])["features"][0][
        "properties"
    ]
...
def test_変換の検証_駐車場が_一部内訳のみだった場合_None内訳のキーを省略する():
    # parringオブジェクト内でも値の無い内訳キーは省略されることを確認する(2.9)
    properties = FacilityProperties(
        name="道の駅テスト",
        kind=FacilityKind.MICHINOEKI,
        pref_code="01",
...
def test_不変性の検証_各データ型が_構築済みだった場合_属性の変更が禁止されている(
    instance, field_name, value
):
