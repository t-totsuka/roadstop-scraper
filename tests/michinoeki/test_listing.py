...
def test_一覧抽出の検証_全要素の座標が解釈できる場合_名称_詳細URL_座標が正しく対応付けて抽出できる():
    ...
    assert result.stubs == (
        StationStub(
            name="道の駅 三笠",
            detail_url="https://www.michi-no-eki.jp/stations/views/18786",
            coordinate=Coordinate(longitude=141.900, latitude=43.123),
        ),
        StationStub(
            name="道の駅 スタープラザ芦別",
            detail_url="https://www.michi-no-eki.jp/stations/views/18787",
            coordinate=Coordinate(longitude=142.400, latitude=43.456),
        ),
    )
    assert result.listed_urls == frozenset(
        {"https://www.michi-no-eki.jp/stations/views/18786", "https://www.michi-no-eki.jp/stations/views/18787"}
    )
...
def test_一覧抽出の検証_一部要素のみ座標が欠落する場合_その1件のみがスキップされ他は抽出される():
    ...
    assert result.stubs == (
        ...
        StationStub(
            name="道の駅 許田",
            detail_url="https://www.michi-no-eki.jp/stations/views/19813",
    ...
    assert result.listed_urls == frozenset(
        {
            ...
            "https://www.michi-no-eki.jp/stations/views/99999",
            "https://www.michi-no-eki.jp/stations/views/19813",
        }
    )
...
def test_一覧抽出の検証_一部要素のみ名称が欠落する場合_その1件はスキップされるがlisted_urlsには含まれる():
    ...
    assert result.listed_urls == frozenset(
        {"https://www.michi-no-eki.jp/stations/views/18786", "https://www.michi-no-eki.jp/stations/views/88888"}
    )
...
def test_一覧抽出の検証_座標が非有限値の場合_その1件のみがスキップされ他は抽出される():
    ...
    assert [stub.name for stub in result.stubs] == ["道の駅 三笠"]
    assert result.skipped_count == 1
    assert "https://www.michi-no-eki.jp/stations/views/77777" in result.listed_urls


def test_一覧抽出の検証_相対URLのdata_linkは一覧ページURL基準の絶対URLへ解決される():
    """実サイトのdata-linkは相対URL(例: "/stations/views/18786")のため、
    そのままfetch_textへ渡すとrequests層でMissingSchemaとなる。stubs・listed_urls
    のいずれも絶対URLへ解決されること、絶対URLのdata-linkはそのまま維持される
    ことを検証する(実サイト疎通確認で検出された回帰のテスト)。
    """
    prefecture = find_prefecture("01")
    html = """
<html>
  <body>
    <main>
      <div class="js-data-box" data-name="道の駅 相対" data-link="/stations/views/18786" data-lat="43.1" data-lng="141.9"></div>
      <div class="js-data-box" data-name="道の駅 絶対" data-link="https://other.example.com/stations/1" data-lat="43.2" data-lng="142.0"></div>
    </main>
  </body>
</html>
"""
    fetcher, _session = _build_fetcher(html)

    result = fetch_station_stubs(fetcher, prefecture)

    assert [stub.detail_url for stub in result.stubs] == [
        "https://www.michi-no-eki.jp/stations/views/18786",
        "https://other.example.com/stations/1",
    ]
    assert result.listed_urls == frozenset(
        {
            "https://www.michi-no-eki.jp/stations/views/18786",
