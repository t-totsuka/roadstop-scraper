...
class _FakeSessionWithFailures(_FakeSession):
    """特定URLのみHTTPエラーステータスを返す偽セッション。

    ``failure_urls``に含まれるURLへは登録ステータス(既定503)の空応答を返し、
    それ以外は``_FakeSession``と同様に事前登録したHTMLを200で返す。一覧ページの
    HTTP取得失敗(``FetchFailedError``)経路の検証に用いる。
    """

    def __init__(self, responses_by_url: dict[str, str], failure_urls: dict[str, int]) -> None:
        super().__init__(responses_by_url)
        self._failure_urls = failure_urls

    def get(self, url, *, timeout, headers):
        if url in self._failure_urls:
            self.calls.append(url)
            return _FakeResponse(self._failure_urls[url], b"", headers={})
        return super().get(url, timeout=timeout, headers=headers)


def test_run_scopeの検証_一部都道府県の一覧ページがHTTP取得失敗でも他の都道府県の処理が継続される(
    tmp_path,
):
    """2.3, 5.2: 一覧ページの取得自体がHTTPエラー(503→FetchFailedError)で失敗する
    場合も、要素0件(ListingUnavailableError)の場合と同様に当該都道府県のみが
    中断され、他の都道府県の処理は継続されることを検証する。FetchFailedErrorは
    ListingUnavailableErrorの兄弟クラスのため、run_prefectureが基底の
    ScrapingEngineErrorで捕捉していないとrun_scope全体が異常終了する(レビュー
    指摘の回帰テスト)。
    """
    tokushima = find_prefecture("36")
    kagawa = find_prefecture("37")
    ...
    session = _FakeSessionWithFailures(responses, {build_search_url(kagawa): 503})
    fetcher = PageFetcher(_fast_config(), session=session)
    resume_store = _make_resume_store(tmp_path)
    resume = UrlResumeTracker(_RESUME_KEY, store=resume_store)
    partial_result_store = _make_partial_result_store(tmp_path)
    ...
    # 香川のみNone・他3県はPrefectureRunResultとなり、run_scopeは例外を送出しない。
    assert len(results) == 4
    assert isinstance(results[0], PrefectureRunResult)
    assert results[1] is None
    assert isinstance(results[2], PrefectureRunResult)
    ...
    # 成功した3県のGeoJSONは出力され、失敗した香川のファイルは書き込まれない。
    for prefecture, expected_name in (
        (tokushima, "道の駅徳島"),
        (ehime, "道の駅愛媛"),
        (kochi, "道の駅高知"),
    ...
    # 6.3: 失敗が残るためresumeはクリアされない。
    reloaded_resume = UrlResumeTracker(_RESUME_KEY, store=resume_store)
    assert reloaded_resume.is_processed(detail_url_tokushima) is True


def test_run_prefectureの検証_前回GeoJSONが破損している場合_当該都道府県のみ中断されファイルが上書きされない(
    tmp_path,
):
    """前回出力ファイルがJSONとして不正な場合、run_prefectureはNoneを返して当該
    都道府県のみを中断し、破損ファイルを空扱いで上書きしない(削除状態・最終
    確認日時の履歴を失わないため)ことを検証する(レビュー指摘の回帰テスト)。
    """
    prefecture = find_prefecture("01")
    detail_url = "https://example.com/stations/views/1"
    responses = {
        build_search_url(prefecture): _listing_html([("道の駅 三笠", detail_url, 43.123, 141.900)]),
        detail_url: _SUCCESS_HTML_TEMPLATE.format(name="道の駅 三笠"),
    }
    fetcher, _session = _make_fetcher(responses)
    resume = _make_resume(tmp_path)
    partial_result_store = _make_partial_result_store(tmp_path)
    ...
    assert result is None
    # 破損ファイルはそのまま残り、上書きされない。
    assert (output_dir / filename).read_text(encoding="utf-8") == corrupted_content
    # index.jsonも更新されない。
    assert not (output_dir / "index.json").exists()


def test_部分結果キャッシュの検証_同一source_urlのadd_featureは置き換えとなり二重登録されない(tmp_path):
    """_PartialResultStore.add_featureの冪等性: 永続化とmark_processedの間の中断で
    再開時に同じ道の駅が再登録されても、source_urlで置き換えられ二重登録に
    ならないことを検証する(レビュー指摘の回帰テスト)。
    """
    prefecture = find_prefecture("01")
    store = _make_partial_result_store(tmp_path)
    partial_store = _PartialResultStore(prefecture, store=store)

    def _feature(name: str) -> FacilityFeature:
        return FacilityFeature(
            coordinate=Coordinate(longitude=141.9, latitude=43.1),
            properties=FacilityProperties(
                name=name,
                kind=FacilityKind.MICHINOEKI,
                pref_code=prefecture.code,
                pref_name=prefecture.name_ja,
                source_url="https://example.com/stations/views/1",
            ),
        )

    partial_store.add_feature(_feature("道の駅 旧"))
    partial_store.add_feature(_feature("道の駅 新"))

    # 同一source_urlは1件に置き換えられ、新しい結果が優先される。
    restored = _PartialResultStore(prefecture, store=store)
    assert [f.properties.name for f in restored.features] == ["道の駅 新"]
